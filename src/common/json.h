...
public:
	...
	// Fallback parser for Apple's ASN.1-DER-encoded "local"/Xcode-managed
	// provisioning profile plist format (used by profiles with
	// IsXcodeManaged/LocalProvision, e.g. free personal-team profiles),
	// which is neither binary plist nor XML plist.
	static bool parse_asn1_plist(const char* pdoc, size_t len, jvalue& root);

private:
	static bool _asn1_read_len(const char*& p, const char* end, size_t& outlen);
	static bool _asn1_read_value(const char*& p, const char* end, jvalue& out);
	static bool _asn1_read_dict_entries(const char* p, const char* end, jvalue& out);
